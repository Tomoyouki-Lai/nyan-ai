...
**喵 AI** 是一款全新的 AI 網頁自動化平台，結合了 **動漫風格角色助理**、**可分享的任務腳本系統（喵卡）**，並以 **社群共創** 為核心理念。  
...
- 前後端分離: index.js: 專注於 UI 呈現與使用者互動，而 background.js 則處理所有核心邏輯、狀態管理和 API 呼叫。
- 高度可配置: 透過 popup.js 和 chrome.storage，使用者可以輕鬆更換 LLM、TTS 服務、角色和功能（透過喵卡），而不需要修改核心程式碼。
- 提示工程核心: promptProcessor.js 是整個系統的靈魂，它將各種動態資訊（個性、歷史、任務）有效地注入到給 LLM 的指令中，從而實現了情境感知和角色扮演。
- 結構化輸出: 整個流程強烈依賴 LLM 能夠穩定輸出特定格式的 JSON。這是實現動畫、語音和互動選項自動化的關鍵。
- 多模態輸出: 最終的結果不僅是文字，還結合了 Live2D 動畫和語音，提供了非常豐富的互動體驗。
